    ...
    producedAt:(ASN1_GENERALIZEDTIME*)producedAt
{
    ...
    reqCertId = OCSP_cert_to_id(EVP_sha1(), requestedCert, issuerCert);
    int* status = NULL;
...
+ (int) checkAIAresponder:(OCSP_BASICRESP*)resp issuer:(X509*)issuer
{
    ...
    for (int i = 0; i < sk_X509_num(OCSP_resp_get0_certs(resp)); i++) {
        ...
        if (retval == X509_V_OK) {
            ...
            if (res > 0) {
                if ((X509_get_extension_flags(responderCert) & EXFLAG_XKUSAGE) &&
                        (X509_get_extended_key_usage(responderCert) & XKU_OCSP_SIGN)) {
                    ret = 1;
                    break;
                }
            }
