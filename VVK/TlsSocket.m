...
- (id) initWithHost:(NSString*)hostname ip:(NSString*)ip port:(NSInteger)port certStrArray:
    (NSArray*)certStrArray
{
    self = [super init];
    ...
    if (self) {
        ...
        CFStreamCreatePairWithSocketToHost(NULL, (__bridge CFStringRef)(ip), (int)port, &readStream,
                                           &writeStream);
...
- (SecCertificateRef) parseCertString:(NSString*)certStr
{
    ...
    NSString* trimmed = [certStr stringByTrimmingCharactersInSet:[NSCharacterSet
                                 characterSetWithCharactersInString:@"\n"]];
    NSArray* array = [trimmed componentsSeparatedByString:@"\n"];
    NSString* base64Str = [[array subarrayWithRange:NSMakeRange(1, [array count] - 2)]
