...
static NSString* const XPATH_TEMPLATE_NODE_BY_ID =
    @"//*[@Id='%s']/descendant-or-self::node() | //*[@Id='%s']//@* | //*[@Id='%s']/namespace::*";
static xmlChar* const XPATH_REFERENCE = (xmlChar*)"//ds:Signature/ds:SignedInfo/ds:Reference";
static xmlChar* const XPATH_SIGNEDINFO = (xmlChar*)
        "//ds:SignedInfo/descendant-or-self::node() | //ds:SignedInfo//@* | //ds:SignedInfo/namespace::*";
static xmlChar* const XPATH_X509CERT = (xmlChar*)"//ds:X509Certificate";
static xmlChar* const XPATH_SIGNATUREVALUE = (xmlChar*)
        "//ds:SignatureValue/descendant-or-self::node() | //ds:SignatureValue//@* | //ds:SignatureValue/namespace::*";
...
- (BOOL) verifyDigests:(xmlDocPtr)doc references:(xmlNodeSetPtr)nodes xpathContext:
...
- (id) initWithData:(NSData*)data electionId:(NSString*)elId
{
    ...
    if (self) {
        ...
        voteFileRegex = [NSRegularExpression regularExpressionWithPattern:voteFileRegexPattern options:0
                                             error:nil];
...
- (BOOL) validateMimeType:(NSData*)data
{
    return [[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] isEqualToString:
                              MIMETYPE_VALUE];
}
...
- (BOOL) isSignatureFile:(NSString*)fileName
{
    NSRegularExpression* regex = [NSRegularExpression regularExpressionWithPattern:SIGNATURE_FILE_REGEX
                                                      options:0 error:nil];
    return [regex numberOfMatchesInString:fileName options:0 range:NSMakeRange(
                      0, [fileName length])] == 1;
}
...
- (BOOL) isVote:(NSString*)fileName
{
    return ![fileName hasPrefix:META_INF_DIR] &&
           ...
           [voteFileRegex numberOfMatchesInString:fileName options:0 range:NSMakeRange(
                              0, [fileName length])] == 1;
}
...
- (BOOL) verifySig:(xmlChar*)sig sigAlg:(xmlChar*)sigAlg key:(EVP_PKEY*)key data:(NSData*)data
{
    ...
    NSData* sigDataRaw = [[NSData alloc] initWithBase64EncodedData:tmp options:
...
- (X509*) xmlCharToX509:(xmlChar*)data
{
    ...
    NSData* der = [[NSData alloc] initWithBase64EncodedData:tmp options:
                                  NSDataBase64DecodingIgnoreUnknownCharacters];
...
{
    NSData* data = [[NSData alloc] initWithContentsOfFile:[[[NSBundle mainBundle] resourcePath]
                                   stringByAppendingString:path]];
    BIO* certBio = BIO_new_mem_buf([data bytes], (int)[data length]);
    DLog(@"Loading SK certificate %@ %lu", path, (unsigned long)[data length]);
